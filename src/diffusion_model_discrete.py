...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    ...
    def training_step(self, data, i):
        ...
        dense_data, node_mask = utils.to_dense(data.x.to_dense(), data.edge_index, data.edge_attr, data.batch)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self.first_pass == True or self.val_counter % self.cfg.general.sample_every_val == 0:
            start = time.time()
    ...
    def kl_prior(self, X, E, y, node_mask):
        ...
        probX = X.float() @ Qtb.X  # (bs, n, dx_out)
        probE = E.float() @ Qtb.E.unsqueeze(1)  # (bs, n, n, de_out)
        proby = y.float() @ Qtb.y if y.numel() > 0 else y.float()
    ...
    def reconstruction_logp(self, t, X, E, y, node_mask):
        ...
        probX0 = X.float() @ Q0.X  # (bs, n, dx_out)
        probE0 = E.float() @ Q0.E.unsqueeze(1)  # (bs, n, n, de_out)
    ...
    def sample_batch(self, batch_id: int, batch_size: int, keep_chain: int, number_chain_steps: int,
        ...
        #print('step 3: sampling noise')

        #time_sample1 = time.time()
        z_T = diffusion_utils.sample_discrete_feature_noise(limit_dist=self.limit_dist, node_mask=node_mask)
        X, E, y = z_T.X, z_T.E, z_T.y
        #print('step 3 took: ' + str(time.time() - time_sample1))
        #print('step 4: iterative sample')
        time_sample2 = time.time()
        ...
        for s_int in reversed(range(0, self.T)):
            ...
            #print('diffusion sampling time ' +str(s_int)+': '+str(time.time()-sample_time))
        ...
        #print('step 4 took: ' + str(time.time()-time_sample1))
        # Sample
        sampled_s = sampled_s.mask(node_mask, collapse=True)
        X, E, y = sampled_s.X, sampled_s.E, sampled_s.y


        #print('keeping chain size:' + str(keep_chain))
        ...
        return predicted_graph_list#molecule_list
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask):
        ...
        prob_X = unnormalized_prob_X / torch.sum(unnormalized_prob_X, dim=-1, keepdim=True)  # bs, n, d_t-1F
