...
class PredefinedNoiseScheduleDiscrete(torch.nn.Module):
    ...
    def __init__(self, noise_schedule, timesteps, device='cpu'):
        super(PredefinedNoiseScheduleDiscrete, self).__init__()
        self.timesteps = timesteps
        self.device = device

        # Generate betas based on the noise schedule
        if noise_schedule == 'cosine':
            ...
            raise NotImplementedError("Unsupported noise schedule")

        self.register_buffer('betas', torch.from_numpy(betas).to(self.device).float())
        ...
        print(f"[Noise schedule: {noise_schedule}] alpha_bar:", self.alphas_bar.cpu().numpy())

    def forward(self, t_normalized=None, t_int=None):
        assert int(t_normalized is None) + int(t_int is None) == 1
        if t_int is None:
            t_int = torch.round(t_normalized * self.timesteps).to(self.device)
